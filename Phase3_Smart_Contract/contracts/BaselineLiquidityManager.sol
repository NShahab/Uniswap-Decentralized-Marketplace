...
/**
 * @title BaselineLiquidityManager
 * @notice قرارداد مدیریت نقدینگی بیس‌لاین که موقعیت‌ها را بر اساس قیمت فعلی بازار تنظیم می‌کند
 */
contract BaselineLiquidityManager is
    Ownable,
    ReentrancyGuard,
    IUniswapV3MintCallback
{
    ...
    function adjustLiquidityWithCurrentPrice() external nonReentrant onlyOwner {
        // محاسبه اطلاعات قیمت و tick های فعلی
        (
            uint256 currentPriceDecimal,
            int24 currentTick,
            int24 targetTickLower,
            int24 targetTickUpper
        ) = _calculateCurrentPriceData();
        ...
        _emitBaselineMetrics(currentPriceDecimal, currentTick, adjusted);
    }

    // تابع کمکی برای محاسبه داده‌های قیمت فعلی - کاهش عمق پشته
    function _calculateCurrentPriceData()
        internal
        view
        returns (
            uint256 currentPriceDecimal,
        ...
        )
    {
        ...
        // محاسبه موقعیت جدید بر اساس قیمت فعلی
        (targetTickLower, targetTickUpper) = _calculateTicks(currentTick);
    ...
    function getCurrentPrice() external view returns (uint256 priceDecimal) {
        uint160 sqrtPriceX96;
        (sqrtPriceX96, ) = _getCurrentSqrtPriceAndTick();
        return _sqrtPriceX96ToPrice(sqrtPriceX96);
    }

    // تابع کمکی برای انتشار متریک‌ها - کاهش عمق پشته در تابع اصلی
    function _emitBaselineMetrics(
        uint256 currentPriceDecimal,
        int24 currentTick,
        bool adjusted
    ) internal {
